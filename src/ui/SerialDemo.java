...
public class SerialDemo extends Frame implements ActionListener {
     ...
     * 串口操作类
     */
    private SerialOperator connection;
    ...
    */
	...
	//添加Load菜单项到File菜单，并指定事件处理函数
	openItem = new MenuItem("Load");
	openItem.addActionListener(this);
	fileMenu.add(openItem);

	//添加Save菜单项到File菜单，并指定事件处理函数
	saveItem = new MenuItem("Save");
	saveItem.addActionListener(this);
	fileMenu.add(saveItem);

	//添加Exit菜单项到File菜单，并指定事件处理函数
	...
	//初始化messagepanel，指定布局模式，添加发送接收文本域
	...
	//初始化串口操作类
	connection = new SerialOperator(parameters);
    ...
    */
	...
	if (cmd.equals("Send")) {
	   this.connection.commSend(this.messageAreaOut.getText());
	   if(this.connection.getResponse()!=null)
	    	this.messageAreaIn.append(this.connection.getResponse());
	}
    }
